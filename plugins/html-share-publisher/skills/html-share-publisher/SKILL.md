...
description: 通过碧橙内容发布 MCP 安全发布、更新、下架或恢复静态网站和文档。当用户要求上传、发布、分享或更新 HTML、静态网站目录、ZIP、Markdown、TXT、Word、PowerPoint、Excel，或下架、恢复已有作品时使用。严格执行钉钉鉴权、本地预检、精准作品定位、影响确认和执行前最终确认。
---

# 碧橙内容发布

使用碧橙内容发布 MCP 执行实际操作。引导用户完成一套稳定、可预期的流程，绝不能在用户不知情时上传。

当用户明确要求撤销、断开或忘记内容发布 AI 授权时，调用 `revoke_authorization`，并说明下次发布时需要重新完成钉钉授权。

## 工具门禁与自动修复

开始操作前，确认当前任务实际提供 `auth_status`、`start_login`、`precheck_package`、`prepare_publish`、`execute_publish`、`prepare_site_status_change` 和 `execute_site_status_change` 等内容发布 MCP 工具。
...
3. 确认准确的本地源文件路径。支持静态网站目录、单个 HTML、ZIP、`.md`、`.txt`、`.docx`、`.pptx` 和 `.xlsx`。旧版 `.doc`、`.ppt`、`.xls` 应提示用户另存为新版格式，不得伪装为已支持。
4. 在讨论是否执行发布之前，调用 `precheck_package` 完成预检。
5. 根据预检结果处理入口和转换信息：
   - 单个 HTML 可直接选择，无需用户手工压缩；MCP 会自动将它封装为以 `index.html` 为入口的 ZIP。必须展示预检中的单文件边界提示：只包含该 HTML；若它引用本地图片、CSS 或 JavaScript，应改为选择包含完整资源的目录或 ZIP。
   - 静态网站存在多个可作为入口的 HTML 文件时，展示完整候选列表和建议入口并让用户明确选择。即使建议入口是 `index.html` 也不能自行确认，更不能根据文件名相似度猜测；确认后在 `prepare_publish` 中同时传入 `entryFile` 和 `entryFileConfirmed: true`。
   - 文档会自动转换为网页，固定使用平台生成的 `index.html`，不得询问用户选择 HTML 入口，也不要传 `entryFileConfirmed: true`。
   - 文档预检返回的格式、页数或工作表数、编码和转换限制必须保留到最终确认摘要中。
...
15. 发布完成后只把 `execute_publish.recipientUrl` 作为提供给接收者的链接，并说明 `recipientAccess`：
    ...
    同时返回 `siteId`、版本号、来源格式和权限摘要。说明公开短码只用于链接，本地精准更新仍绑定真实 `siteId`。本地清单文件：目录使用 `.htmlshare.json`；单个 HTML、ZIP 和文档使用与源文件对应的 `名称.htmlshare.json`，从而允许同一目录存在多个作品。

## 安全规则

- 首次展示最终确认摘要的同一轮中，绝不能调用 `execute_publish`。
- `precheck_package` 和 `prepare_publish` 都不得上传源文件；只有用户确认后调用 `execute_publish` 才能上传。
...
- `prepare_publish` 和 `prepare_site_status_change` 返回的计划均与服务端签名令牌绑定且 15 分钟失效；不得修改计划文件、复用过期计划或把一个计划用于其他作品。
- 向用户说明：更新会创建新版本并保留稳定链接，不会抹掉历史版本。
- 单个 HTML 引用了本地图片、CSS 或 JavaScript 时，必须提示用户改为选择包含完整资源的目录或 ZIP，不能声称这些依赖会自动上传。
- 文档转换失败时不得改传 ZIP 或伪造成功；应展示工具错误并说明没有创建新版本。
